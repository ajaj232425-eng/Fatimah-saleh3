# إنشاء نسخة جديدة من HTML مع تعديل CSS لمنع تكرار الخلفية
html_file_path_fixed = "/mnt/data/index_fixed.html"

html_content_fixed = f"""<!DOCTYPE html>
<html lang="ar">
<head>
<meta charset="UTF-8">
...
body{{
    ...
    /* الخلفية تظهر مرة واحدة فقط */
    background-image: url('data:image/png;base64,{encoded_image}');
    background-repeat: no-repeat;
    background-position: top center;
    background-size: contain;
}}
.header{{ background:white; padding:15px; border-radius:12px; margin-bottom:15px; opacity:0.95; }}
.logo{{ width:90px; margin-bottom:10px; }}
.header h2{{ margin:0; color:#0a6ebd; }}
.header h3{{ margin:5px 0 0; font-weight:normal; color:#333; }}
.container{{ background:white; padding:25px; border-radius:15px; box-shadow:0 0 15px rgba(0,0,0,0.2); max-width:350px; margin:auto; opacity:0.95; }}
h1{{ color:#ff6b6b; }}
...
# حفظ HTML المعدل
with open(html_file_path_fixed, "w", encoding="utf-8") as f:
    f.write(html_content_fixed)

# إنشاء ZIP جديد يحتوي على index.html المعدل
zip_file_path_fixed = "/mnt/data/Child_Age_Calculator_Fixed.zip"
with zipfile.ZipFile(zip_file_path_fixed, 'w') as zipf:
    zipf.write(html_file_path_fixed, arcname="index.html")
